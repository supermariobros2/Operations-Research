...
# Show the foods we eat
for f in F:
    if X[f].x > 0.00001:
        print(Food[f],"=",X[f].x)

# Side note:
# Dropping the upper limits reduces the cost substantially
# Which are the nutrients whose limits are making the diet more expensive?
for n in N:
    consumed = sum(NV[f][n]*X[f].x for f in F)
    if consumed > DMAX[n]:
        print(Nutrients[n],"=",consumed,"(",DMAX[n],")")
